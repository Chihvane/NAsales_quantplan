from __future__ import annotations

import json
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from quant_framework.charts import generate_part3_chart_assets  # noqa: E402
from quant_framework.cleaners import (  # noqa: E402
    normalize_part3_compliance_export,
    normalize_part3_logistics_export,
    normalize_part3_rfq_export,
    normalize_part3_shipment_export,
    normalize_part3_suppliers_export,
    normalize_part3_tariff_export,
)
from quant_framework.io_utils import write_json  # noqa: E402
from quant_framework.part3 import build_part3_quant_report  # noqa: E402
from quant_framework.part3_pipeline import (  # noqa: E402
    DEFAULT_PART3_ASSUMPTIONS,
    build_part3_dataset_from_directory,
)

RAW_DIR = ROOT / "examples"
OUTPUT_DIR = ROOT / "examples" / "cleaned_part3_demo"


def main() -> None:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    summaries = {
        "suppliers": normalize_part3_suppliers_export(
            RAW_DIR / "raw_part3_suppliers_export.csv",
            OUTPUT_DIR / "suppliers.csv",
        ),
        "rfq_quotes": normalize_part3_rfq_export(
            RAW_DIR / "raw_part3_rfq_export.csv",
            OUTPUT_DIR / "rfq_quotes.csv",
        ),
        "compliance_requirements": normalize_part3_compliance_export(
            RAW_DIR / "raw_part3_compliance_export.csv",
            OUTPUT_DIR / "compliance_requirements.csv",
        ),
        "logistics_quotes": normalize_part3_logistics_export(
            RAW_DIR / "raw_part3_logistics_export.csv",
            OUTPUT_DIR / "logistics_quotes.csv",
        ),
        "tariff_tax": normalize_part3_tariff_export(
            RAW_DIR / "raw_part3_tariff_export.csv",
            OUTPUT_DIR / "tariff_tax.csv",
        ),
        "shipment_events": normalize_part3_shipment_export(
            RAW_DIR / "raw_part3_shipment_export.csv",
            OUTPUT_DIR / "shipment_events.csv",
        ),
    }
    dataset = build_part3_dataset_from_directory(OUTPUT_DIR)
    report = build_part3_quant_report(dataset, DEFAULT_PART3_ASSUMPTIONS)
    report_path = write_json(OUTPUT_DIR / "part3_report.json", report)
    chart_paths = generate_part3_chart_assets(report, OUTPUT_DIR / "charts")
    print(
        json.dumps(
            {
                "cleaning": summaries,
                "report_json": str(report_path),
                "charts": chart_paths,
                "validation": report["validation"]["summary"],
            },
            ensure_ascii=False,
            indent=2,
        )
    )


if __name__ == "__main__":
    main()
