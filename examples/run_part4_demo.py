...
from quant_framework.charts import generate_part4_chart_assets  # noqa: E402
...
DATA_DIR = ROOT / "examples" / "part4_demo"
OUTPUT_DIR = ROOT / "examples" / "output_part4"


def main() -> None:
    dataset = build_part4_dataset_from_directory(DATA_DIR)
    report = build_part4_quant_report(dataset, DEFAULT_PART4_ASSUMPTIONS)
    report_path = write_json(OUTPUT_DIR / "part4_report.json", report)
    chart_paths = generate_part4_chart_assets(report, OUTPUT_DIR / "charts")
    print(
        json.dumps(
            {
